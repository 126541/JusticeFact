...
def run_reason():
    ...
    keyword_prompt = "Goal:As an organizer, your task is to extract a set of keywords or search phrases based on the content of the news report and the {} stance (thinking of the news as {}).These keywords will be used to perform real-time web searches that help determine whether the news is a rumor.\n\n"
    ...
    keyword_prompt += "Requirement 3: Strictly stand on the standpoint of {} to generate **accurate and verifiable** keywords or search phrases to help {}'s reasoning about whether the news is a rumor.\n"
    ...
    prosecutor_prompt = "Goal: As a prosecutor, based on your knowledge base, filter online search results, and combined with the reasoning perspective and news content, it is forcibly inferred that the news is a rumor.\n"
    ...
    prosecutor_prompt += "Requirement 4: Filtering requirements: The filtered results must be highly relevant to the reasoning perspective and news content, and meet the purpose of determining that the news is a rumor. \n"
    ...
    prosecutor_prompt += "Requirement 6: The generated reasoning content must strongly support that the news is a rumor, and cannot contain uncertain expressions or weak judgments such as 'possibly', 'may be', or references to authenticity. \n"
    ...
    defense_prompt = "Goal: As a defense lawyer, based on your knowledge base, filter online search results, and combined with the reasoning perspective and news content, it is forcibly inferred that the news is not a rumor.\n"
    ...
    defense_prompt += "Requirement 4: Filtering requirements: The filtered results must be highly relevant to the reasoning perspective and news content, and meet the purpose of determining that the news is not a rumor. \n"
    ...
    defense_prompt += "Requirement 6: The generated inference content must strongly support that the news is not a rumor, and cannot contain uncertain expressions or weak judgments such as 'possibly', 'may be', or references to authenticity. \n"
    ...
    prosecutor_prompt1 = "Goal: As a prosecutor, my previous statement was not strong enough. This time, I will still use my knowledge base, as well as the news information and these reasoning angles to **refute** the defense lawyer's statement by filtering real-time search content. This statement must prove the news is a rumor in a stronger and more convincing way than before. Your stance must consistently assert that the news is a rumor.\n"
    ...
    prosecutor_prompt1 += "Requirement 7: The generated reasoning must demonstrate clearly that the news is a rumor. Do not use uncertain expressions or words like 'authenticity'. \n"
    ...
    defense_prompt1 = "Goal: As a defense lawyer, my previous statement was not strong enough. This time, I will still use my knowledge base, combined with news information and these reasoning angles, to refute the prosecutor's statement by filtering real-time search content. This statement must prove more strongly and convincingly than the previous one that the news is not a rumor. Your position must be consistent in insisting that the news is not a rumor.\n"
    ...
    defense_prompt1 += "Requirement 7: The generated reasoning must clearly show that the news is not a rumor. Do not use uncertain expressions or words like 'authenticity'. \n"
    ...
    judge_prompt = "Goal: You are the chief judge overseeing the entire debate process. You have access to the full news content, the prosecution's and defense's evidence sources, all rounds of statements from both sides, and the final comments from all jury members. Your role is to make a final, impartial judgment on whether the news is a rumor, providing clear reasoning based on all presented information.\n"
    ...
    judge_prompt += "Requirement 9:As the Chief Judge, please analyze the evidence, arguments, and jury comments to decide whether the news is a rumor. Please provide a detailed explanation of your decision (maximum 100 words). with no line breaks or special characters.\n"
    judge_prompt += "Requirement 10: Make a final binary judgment: output 1 if the news is a rumor, 0 if it is not a rumor.\n"
    judge_prompt += "Requirement 11: The output format is: what is the reasoning and what is the final_verdict. For example, reasoning: 'The news that......'; final_verdict: '1'.\n"
    judge_prompt += "Requirement 12: The reasoning content and final_verdict must be separated by a semicolon.\n"

    m = 0.5

    credibility_score_prompt = "Goal: Evaluate the credibility of the news as NON-RUMOR according to the current {} argument and the retrieved evidence.\n"
    ...
    credibility_score_prompt += "Requirement 4: Return a credibility score V in [0,1], where 0 means the news is definitely a rumor and 1 means the news is definitely non-rumor.\n"
    ...
    unshuff_data = pd.read_csv(r'D:\fake news\dateset\twitter16\xu.csv')
                                                                                                                                  
    
    data = unshuff_data.sample(frac=1, random_state=42)

    save_path = r'D:\fake news\dateset\twitter16\xu1.csv'
    ...
    for index, row in tqdm(data.iterrows(), total=data.shape[0]):
        ...
        def select_best_professions_with_prompts(input_text, profession_keywords, get_prompts_func, top_k=7):
            ...
            prompt = f"""
            Goal: You are a classifier. Based on the category and theme of the news, please select exactly seven occupations that are the **most relevant** to the news **from the provided jury list ONLY**.
