...
def run_reason():
    ...
    for index, row in tqdm(data.iterrows(), total=data.shape[0]):
        ...
        def select_best_professions_with_prompts(input_text, profession_keywords, get_prompts_func, top_k=9):
            ...
            # Step 2: 构造 prompt
            prompt = f"""
            Goal: You are a classifier. Based on the category and theme of the news, please select exactly 9 occupations that are the **most relevant** to the news **from the provided jury list ONLY**.
            ...
            Requirement 4: The final output must be exactly 9 occupations, and they must be selected **strictly from the jury list**. Do NOT add general terms like "engineer", "chemist", or other professions not in the list.This is a strict classification task. If you output even one word not in the list, the task fails.

            Requirement 5: Output format must be exactly:
            ["occupation_1", "occupation_2", ..., "occupation_9"]
        ...
        def map_second_round_prompts(selected, get_prompts_func):
            ...
            prompt_map = {}
            for i in range(min(9, len(selected))):
        ...
        #控方反驳第二轮
        prosecutor_content_1 = prosecutor_prompt1.format(prosecutor_content_1, search_text_combined_2, view_content, input_text ,defense_content_1,first_round_judgements)
        prosecutor_content_1 = generate_res(prosecutor_content_1)
        #辩方反驳第二轮
        defense_content_1 = defense_prompt1.format(defense_content_1, search_text_combined_2, view_content, input_text,prosecutor_content_1,first_round_judgements)
        defense_content_1 = generate_res(defense_content_1)
